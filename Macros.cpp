...
void   __fastcall SelectInstrForMacrosExpans(); // выбор типа инструкции для расширения препроцессором
...
void   __fastcall handOpd_asMassive_1D( char* Opd, INT i ); // // обработка поля-операнда в виде псевдомассива
void   __fastcall handRes_asMassive_1D( char *Result, INT i ); // обработка поля-результата в форме псевдомассива
//
...
// handRes_asMassive_1D - обработчик поля результата как 1D-массив (псевдомассив)
...
//
  if( flagMacroTitle && str[0] != '}' ) // пока не было "}" в строке имеем flagMacroTitle=true
  {
...
//
////////////////////////////////////////////////////////////////////////////////
   SelectInstrForMacrosExpans(); // выбор типа инструкции для расширения препроцессором
////////////////////////////////////////////////////////////////////////////////
...
//
} // --------- конец SelectInstrForMacrosExpans --------------------------------
...
////////////////////////////////////////////////////////////////////////////////
  if( !strchr(Res, '[') && !strchr(Res,']') ) // в Res нет и '[' и ']' ---------
   ...
   handRes_asMassive_1D( Res, i ); // обрабатываем результат, если он псевдомассив
////////////////////////////////////////////////////////////////////////////////
...
//
 if( Rule_TokenUse ) // использовать токен для имени операнда Opd
 {
  sprintf( w, "%s:%d:%d", Opd,numbFor,i-1 ); // имя переменной + id for + i (токен)
...
//
  if( !strncmp( str, "for[", 4 ) && // если в начале str находится 'for[' (первые 4 символа) 'и'
       sscanf( str, "for[%c:%c]=%d,%d,%d:%d,%d,%d{", &cI,&minI,&maxI,&dI, // все 8 полей корректно прочитаны..!
                                                     &cJ,&minJ,&maxJ,&dJ ) == 8 )
  { // начало if( !strncmp( str, "for[", 4 ) && ...
//
