...
pattern[] = "[A-Z]{3}" // мнемоника команды (ровно 3 заглавных, латиница)
            "[ A-za-x0-9_]{1,32}"  // пробел + операнд_1 (1-32 символа)
            "[ ,A-za-x0-9_]{1,32}" // пробел + запятая + операнд_2 (1-32 символа)
            "[ ,A-za-x0-9_]{1,32}" // пробел + запятая + операнд_3 (1-32 символа)
            "[ ,A-za-x0-9_]{1,32}" // пробел + запятая + предикат (1-32 символа)
            "[ ;A-za-x0-9_]{0,}"   // пробел + точка с запятой + комментарий
            ; // шаблон поиска
//
...
//
#define do_Stop \
{ F1->BitBtn_Run->Enabled = false; /* "выключили" все кнопки режма ВЫПОЛНЕНИЕ */ \
...
bool   __fastcall Test_aResult_Eq_aOperand(INT i_Set); // тест на совпадение адреса результата с адресами одного или обоих операндов
...
void   __fastcall ReplaceEqualLengthSubstring( char *String, char *OldSubstring, char *NewSubstring ); // замена "\" на "/"
//
...
void  __fastcall Upload_Data( int Rule); // вЫгрузить файлы на сервер (в зависимости от Rulq)
//==============================================================================
...
// double t_Start; // момент времени начала выполнения инструкции
 INT tick_Start; // момент (тик, такт) старта выполнения инструкции
} M_P, *Mem_Proc=NULL;
INT max_Proc = _128, // первоначальный захват
...
INT max_Instruction = _128,   // первоначальный захват
...
INT Really_Select = 0; // текущее значение
//
/////////////////////////////////////////////////////////////////////////////
...
struct mb {
 INT i_Set; // номер ГКВ-инструкции из пула инструкций
...
INT max_Buffer = _128,  // первоначальный захват
...
////////////////////////////////////////////////////////////////////////////////
INT  FileSizeFromServer; // размер файла для выгрузки с сервера
...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
void __fastcall // двойной щелчок мыши на SG (фрейм "Данные", тип TStringGrid)
TF1::SG_Data_Info(TObject *Sender)
{
 if( SG_Data->Cells[SG_Data->Col][SG_Data->Row] == "" ) // ячейка пустая !..
  return;
//
 char str[_128] = "\0";
//
...
//
 mR->Clear(); //  // очистили фрейм вывода протокола расчёта
//
...
////////////////////////////////////////////////////////////////////////////////
   ...
   {
    case 1: mS->Cells[2][i+1] = Mem_Instruction[i].aOp1; // адрес первого (и единственного) операнда
            mS->Cells[3][i+1] = "\0";
            mS->Cells[5][i+1] = Mem_Instruction[i].aPredicat; // адрес предиката
            mS->Cells[4][i+1] = Mem_Instruction[i].aResult; // адрес результата
            mS->Cells[7][i+1] = Mem_Instruction[i].Comment; // комментарий
//
            break;
//
    case 2: mS->Cells[2][i+1] = Mem_Instruction[i].aOp1; // адрес первого операнда
            mS->Cells[3][i+1] = Mem_Instruction[i].aOp2; // адрес второго операнда
            mS->Cells[5][i+1] = Mem_Instruction[i].aPredicat; // адрес предиката
            mS->Cells[4][i+1] = Mem_Instruction[i].aResult; // адрес результата
...
//
   if( is_PredicatOrSET( Set ) ) // это SET или ПРЕДИКТОР (поле флага предиктора ПУСТОЕ)
    mS->Cells[5][i+1] = "\0"; // ... поле ПРЕДИКАТА - ПУСТОЕ..!
//
...
//
 for( INT i=0; i<Really_Data; i++ ) // теперь ВИЗУАЛИЗИРУЕМ инструкции (вывод в SG_Data)...
  {
...
//
 if( is_ResultIsPredicat( Mem_Data[i].Addr ) ) // это имя флага-предиката ?
...
//
 switch( Rule ) // описание Rule см. выше
 {
  case 0:
  ...
  case 1:
    for( INT i=0; i<Really_Set; i++ ) // по всем инструкциям данной программы
     if( strcmp( Mem_Instruction[i].Set,"SET" ) ) // если это не SET...
      countSets++;
//
    snprintf(tmp,sizeof(tmp), "\n-W- Программа завершается: в течение [%d] тактов не выявлено ни одной ГКВ-инструкции (выполнено/всего инструкций: %d/%d исключая SET) -W-",
             waitAboveOfEndLastExecuteSet, mTpr->Count, countSets );
    SBM0->Text = tmp; // вывод текста в StatusBarMain
    AddLineToProtocol( tmp );
//
    if( !mTpr->Count ) // не выполнилось ничего кроме, возможно, SET
    {
...
TF1::Run_Calculations(TObject *Sender)
{
 ...
 Out_Data_SBM1(); // вывод данных в среднюю часть StatusBar -------------------
//
 Start_DF( 0 ); // старт без перемешивания инструкций
...
//
 Out_Data_SBM1(); // вывод данных в среднюю часть StatusBar --------------------
//
 Clear_AllTableInstructions(); // очистили все ячейки таблицы визуализации инструкций
//
...
//
 if( Mode ) // требуется перемешать инструкции в ПАМЯТИ_ИНСТРУКЦИЙ
...
//
 if( !Test_All_Operands() ) // если тест всех операндов на возможность ГКВ неуспЕшен..1
 {
  do_Run // "включили" все кнопки Выполнение
  return; // заканчиваем выполнение программы
 }
//
 Upload_Data( 0 ); // вЫгрузить SET-файл на сервер ( Rule == 1 )
//
...
//
 for( INT i=0; i<max_Proc; i++ ) // по списку АИУ
...
//   if(i_Set != BUFFER_EMPTY) // если буфер не пуст...
   ExecuteInstructions_Except_SET(i_Set); // выполнить инструкцию i_Set ( кроме SET )
  }
//
...
//
 for( INT i=0; i<Really_Set; i++ ) // по всем инструкциям...
  {
...
//
 if( Really_Select <= 0 ) // ничего не надо выделять цветом
  return;
//
 for( INT i=0; i<Really_Select; i++ ) // по содержимому списка выделенных ячеек
 {
//
  if( !Sel_Cell[i].Col && !Sel_Cell[i].Row ) // если оба нулевые...
   continue;
//
  if( ACol!=Sel_Cell[i].Col || ARow!=Sel_Cell[i].Row ) // это не текущая ячейка!..
   continue;
//
...
//
 } // конец цикла for( INT i=0 ... по содержимому массива Sel_Cell[] -----------
//
} //----------------------------------------------------------------------------
...
//
  for( INT j=0; j<Really_Set; j++ ) // ... опять же по всем строкам Mem_Instruction[] для поиска по операндам
  {
...
//
  for( INT j=0; j<Really_Set; j++ ) // ... опять же по всем строкам Mem_Instruction[] для поиска по РЕЗУЛЬТАТАМ
  {
...
//
  if( SpeculateExec && // задан режим спекулятивного выполнения
      Mem_Instruction[i].fSpeculateExec ) // цвет ячейки результата СПЕКУЛЯТИВНО выполнившейся инсрукции
  {
   Sel_Cell[Really_Select].Col = 4; // столбец РЕЗУЛЬТАТ
   Sel_Cell[Really_Select].Row = i + 1;
...
//
 for( INT i=0; i<Really_Set; i++ ) // по всем строкам Mem_Instruction[i]
 {
  if( is_SET( Mem_Instruction[i].Set ) ) // инструкция SET не рассматривается...
   continue;
//
...
//
 for( INT i=0; i<Really_Set; i++ ) // по всем инструкциям в буфере
 {
...
//
  for( INT j=0; j<Really_Set; j++ ) // по всем инструкциям в буфере
  {
...
//
} // ------ конец Clear_AllTableInstructions -----------------------------------------------
...
//
  for( INT i_tick=0; i_tick<=tick_End; i_tick++ ) // по всем тактам программы
  {
...
//
  } // конец for( INT i_tick=0; i_tick<=tick_End; i_tick++ ) // по всем тактам программы
//
 fclose( fptr ); // закрыли файл
//
...
//  if( (c=='\n') || (c=='\0') ) // управляющий символ или конец строки
  {
   ...
   strcpy( buf, "\0" ); // очистили строку для дальнейшей работы
  }
//
...
//
 if( mS->Col != 4 ) // это вообще не столбец результата... пропускаем !!!
  return;
//
 strcpy( aResult, Mem_Instruction[mS->Row-1].aResult ); // запомнили адрес aResult
//
 Really_Select = 0; // счётчик заполненных в Sel_Cell ячеек (глобал)
//
 for( INT i=0; i<Really_Set; i++ ) // по всем строкам Mem_Instruction[i]
...
//
  if( is_SET( Set ) ) // инструкцию SET не рассматриваем !!!
    continue;
//
// ----- поле предиката (столбец 5) внутри switch не обрабатываем --------------
//
  switch( Get_CountOperandsByInstruction(Set) ) // ... число входных операндов инструкции Set
  {
   case 1: if( !strcmp(aResult, Mem_Instruction[i].aOp1) ) // есть совпадение aResult с aOp1 (один операнд) ?
           {
            Sel_Cell[Really_Select].Col = 2; // столбец 2
            Sel_Cell[Really_Select].Row = i + 1;
            Sel_Cell[Really_Select].clBackground = DCT.clOperandOperation;
            Sel_Cell[Really_Select++].clSymbol   = clBlack;
//
            snprintf(tmp,sizeof(tmp), " %d(1|1)", i); // 1-й (и единственный) операнд
            strcat(str, tmp);
//
            snprintf(tmp,sizeof(tmp), " %d(*|1)", i); // 1-й (и единственный) операнд
            strcat(str, tmp);
           }
//
           break;
//
   case 2: if( !strcmp(aResult, Mem_Instruction[i].aOp1) ) // есть совпадение aResult с aOp1  (два операнда) ?
           {
            Sel_Cell[Really_Select].Col = 2; // столбец 2
            Sel_Cell[Really_Select].Row = i + 1;
            Sel_Cell[Really_Select].clBackground = DCT.clOperandOperation;
...
//
           if( !strcmp(aResult, Mem_Instruction[i].aOp2) ) // есть совпадение aResult с aOp2 (два операнда) ?
           {
            Sel_Cell[Really_Select].Col = 3; // столбец 3
            Sel_Cell[Really_Select].Row = i + 1;
            Sel_Cell[Really_Select].clBackground = DCT.clOperandOperation;
            Sel_Cell[Really_Select++].clSymbol   = clBlack;
//
...
//
////////////////////////////////////////////////////////////////////////////////
// ----- поле предиката (столбец 5) внутри switch не обрабатывался -------------
//
// ----- начинаем обработку поля предиката (столбец 5) -------------------------
//
  strcpy( aPredicat, Mem_Instruction[i].aPredicat ); // запомнили для удобства сравнений
//
  if( !strcmp(aResult,aPredicat) || // прямое совпадение "ИЛИ" ...
    ( ( aPredicat[0]==symbolNot_1 || aPredicat[0]==symbolNot_2 ) && // первый символ - отрицание
       !strcmp(aResult,&aPredicat[1] ) ) ) // совпадение начиная со второго символа
  {
   Sel_Cell[Really_Select].Col = 5; // столбец 5 (значение предиката)
   Sel_Cell[Really_Select].Row = i + 1;
   Sel_Cell[Really_Select].clBackground = DCT.clOperandOperation;
   Sel_Cell[Really_Select++].clSymbol   = clBlack;
//
   snprintf(tmp,sizeof(tmp), " %d(Pred)", i); // операнд-предикат
   strcat(str, tmp);
//
  }
//
 } // конец цикла for( INT i=0; i<Really_Set; i++ ) // по всем строкам Mem_Instruction[i]
////////////////////////////////////////////////////////////////////////////////
//
 mS->Repaint();
//
...
//
 ...
 Row = mS->Row;
//
 if( Col==2 ) // первый операнд
  strcpy(aOp1, Mem_Instruction[mS->Row-1].aOp1); // запомнили адрес aOp1
 else
 if( Col==3) // второй операнд
  strcpy(aOp2, Mem_Instruction[mS->Row-1].aOp2); // запомнили адрес aOp2 (может не быть!!!)
 else
 if( Col==5 ) // операнд-предикат
  strcpy(aPredicat, Mem_Instruction[mS->Row-1].aPredicat); // запомнили адрес aPredicat
//
 strcpy( Set, Mem_Instruction[Row-1].Set ); // взяли мнемонику выделенной инструкции
 n_Op = Get_CountOperandsByInstruction(Set); // число её входных операндов
//
 Really_Select = 0; //  счётчик заполненных в Sel_Cell ячеек (глобал)
//
 for( INT i=0; i<Really_Set; i++ ) // по всем строкам Mem_Instruction[]
...
//
  switch( Col )  // по номеру колонки таблицы
  {
   case 2: // первый входной операнд ( aOp1, столбец 2 )
           if( !strcmp(aOp1,aResult) ) // есть совпадение aOp1 с aResult ?
           {
            Sel_Cell[Really_Select].Col = 4; // результат - столбец 4
            Sel_Cell[Really_Select].Row = i + 1;
            Sel_Cell[Really_Select].clBackground = DCT.clOperandOperation;
            Sel_Cell[Really_Select++].clSymbol   = clBlack;
//
...
//
   break;
//
   case 3: // второй входной операнд ( aOp2, столбец 3 )
           if( n_Op==2 ) // второй (из двух) входной операнд ( aOp2, столбец 3 )
            if( !strcmp(aOp2,aResult) ) // есть совпадение aOp2 с aResult ?
           {
            Sel_Cell[Really_Select].Col = 4; // результат - столбец 4
            Sel_Cell[Really_Select].Row = i + 1;
            Sel_Cell[Really_Select].clBackground = DCT.clOperandOperation;
...
//
   break;
//
   case 5: // операнд-предикат ( aPredicat, столбец 5 )
           if( !strcmp(aResult,aPredicat) || // прямое совпадение "ИЛИ" ...
            ( ( aPredicat[0]==symbolNot_1 || aPredicat[0]==symbolNot_2 ) && // первый символ - отрицание
               !strcmp(aResult,&aPredicat[1] ) ) ) // совпадение начиная со второго символа
           {
...
//
 } // конец цикла for( INT i=0; i<Really_Set; i++ ) // по всем строкам Mem_Instruction[]
////////////////////////////////////////////////////////////////////////////////
//
 mS->Repaint();
//
...
{ // вызывается при всплывании SG_PopUpMenu
} //----- конец TF1::OnPopup_Determine_Links -----------------------------------
...
{ // принудительный вызов меню по правой кнопке мыши при SG_PopUpMenu->AutoPopup=false
 int Col,Row;
//
...
//
 if( !strlen( p ) ) // пустая ячейка
  return;
//
 if( !strcmp( p , trueLowerCase ) || // в ячейке не 'true'
     !strcmp( p , falseLowerCase ) ) // в ячейке не 'false'
  return;
//
// ----- настраиваем активность вариантов всплывающего меню SG_PopUpMenu -------
//
 OR_1->Enabled = false; // сделать неактивными оба варианта всплывающего меню
 RO_1->Enabled = false;
//
 if( Col==4 ) // выбран столбец РЕЗУЛЬТАТ
  RO_1->Enabled = true; // активизировать вариант меню "Операнды->Результат"
//
 if( Col==2 || Col==3 || Col==5 ) // выбран столбец ОПЕРАНД-1 "ИЛИ" ОПЕРАНД-2 "ИЛИ" GHTLBRFB
  OR_1->Enabled = true; // активизировать вариант меню "Результат->Операнды"
//
